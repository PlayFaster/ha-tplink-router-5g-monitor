...
def test_binary_sensor_is_on_best_connection(mock_coordinator, mock_config_entry):
    ...
    sensor = TPLinkRouterBinarySensor(
        mock_coordinator, mock_config_entry, description
    )

    # 1. Both active
    mock_coordinator.data = {"extra_lte_status": {"endc_support": "1", "nr_dl_mod": "256QAM"}}
    assert sensor.is_on is True

    # 2. Only one active
    mock_coordinator.data = {"extra_lte_status": {"endc_support": "0", "nr_dl_mod": "256QAM"}}
